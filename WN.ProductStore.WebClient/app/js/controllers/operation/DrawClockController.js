...
define(["app",
    ...
    var deps = ['$scope', '$window', '$interval', '$timeout', '$stateParams', '$state', 'UserService', 'GeolocationService', 'OperationalActivityResource'];

    function controller($scope, $window, $interval, $timeout, $stateParams, $state, UserService, GeolocationService, OperationalActivityResource) {

        var taskId = $stateParams.drawId;
        ...
        init();
        function init() {
            OperationalActivityResource.getOperateClockUserList(taskId).success(function (data) {
        ...
        function setClockState(taskClockState) {
            if (taskClockState == 1) {
                ...
                GeolocationService.getCurrentPosition().then(function (location) {
                    OperationalActivityResource.setActivityClockState(taskId, taskClockState, location.longitude, location.latitude).success(function (data) {
                        $scope.taskClockState = data.taskClockState;
                        if (data.taskClockUserId && user.userId != data.taskClockUserId) {
                            $scope.isOther = true;
                            setTime(data.taskClockStartDate, data.taskClockFinishDate);
                ...
                OperationalActivityResource.setActivityClockState(taskId, taskClockState, 0,0).success(function (data) {
                    $scope.taskClockState = data.taskClockState;
                    if (timer) {
                        $interval.cancel(timer);
                    }
    ...
    app.lazy.controller("DrawClockController", controller);
});
